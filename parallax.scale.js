$.fn.extend({
  parallaxScale: function(options) {
		...
		$.each(options, function(i, option) {
			...
			offset += (options[i+1].start - option.start) * option.scale;
		});
		
		var self = this;
		this.parallax(start, function(e, start, position, scroll) {
			var change = config[start].offset + (position * config[start].scale);
				
			self.transform({scale: change}).trigger('parallax-scale', [change, start, scroll, position]);
		});
