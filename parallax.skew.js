$.fn.extend({
  parallaxSkew: function(options) {
		...
		$.each(options, function(i, option) {
			...
			offset += (options[i+1].start - option.start) * option.skew;
		});
		
		var self = this;
		this.parallax(start, function(e, start, position, scroll) {
			var change = config[start].offset + (position * config[start].skew);
			self.transform({skew: change}).trigger('parallax-skew', [change, start, scroll]);
		});
